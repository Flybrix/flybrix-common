describe('COBS service', function() {
    ...
    describe('.Reader()', function() {
        ...
        describe('.readBytes()', function() {
            it('exists', function() {
                expect(reader.readBytes).toBeDefined();
            });

            it('handles trivial packet', function(done) {
                reader.readBytes(
                    [1, 1, 1, 1, 1, 1, 1, 0], function(data) {
            ...
            it('handles full packets', function(done) {
                reader.readBytes(
                    ...
                    function(data) {
                        expect(data).toEqual(new Uint8Array([3, 0x12, 0xA4, 0x03, 0x1A, 13, 0, 77]));
                        done();
                    },
                    errFail);
            });
            ...
            it('handles partial packets', function(done) {
                reader.readBytes([8, 236, 3, 0x12], cbFail, errFail);
                reader.readBytes(
            ...
            it('handles consecutive packets', function(done) {
                var counter = 0;
                var exp_data = [
                    new Uint8Array([3, 0x12, 0xA4, 0x03, 0x1A, 13, 0, 77]),
                    new Uint8Array([7, 0x44, 0x22, 0x11, 0x77, 72, 55, 0, 18, 14, 0, 23]),
                ];
                reader.readBytes(
                    [
                        8, 236, 3, 0x12, 0xA4, 0x03, 0x1A, 13, 2, 77, 0,
                        9, 115, 7, 0x44, 0x22, 0x11, 0x77, 72, 55, 3, 18, 14, 2, 23, 0,
                    ],
                    function(data) {
                        expect(data).toEqual(exp_data[counter]);
                        if (++counter === 2) {
                            done();
                        }
                    },
            ...
            it('handles large non-zero packets', function(done) {
                var data = new Uint8Array(303);
                var output = new Uint8Array(299);
                for (var i = 0; i < 303; ++i) {
                    data[i] = 5;
                }
                for (var i = 0; i < 299; ++i) {
                ...
                data[255] = 47;
                data[302] = 0;
                reader.readBytes(data, function(data) {
            ...
            it('refuses bad framing', function(done) {
                reader.readBytes([12, 3, 2, 14, 8, 1, 9, 0], cbFail,
                                      function(reason) {
            ...
            it('refuses bad checksum', function(done) {
                reader.readBytes([7, 1, 2, 2, 2, 2, 2, 0], cbFail,
                                      function(reason) {
            ...
            it('fails on buffer overflow', function(done) {
                ...
                short_reader.readBytes(data, cbFail, function(reason) {
